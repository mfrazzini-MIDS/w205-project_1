...
setwd("/Users/mauracullen/Documents/UCB_MIDS/W205_StoringRetrievingData/w205-project_1/")
cwd = getwd()

plot_all = TRUE;
save_plots = TRUE;
imagepath = paste(cwd,"images",sep = "/")
...
df = read.csv(paste(cwd,"model_summary.csv",sep = "/"))
df = as_tibble(df)
...
p <- ggplot(data = test_df, mapping = aes(x = atc_rate, y = y_preds)) + 
  ...
  ggtitle(title_str)
print(p)
ggsave(filename= paste0(imagepath, '/lm_scatter_best_model', length(curr_m$coefficients), '.png'), plot = p)
